...
NB_LOOKBACK_DAYS = 30 # This needs to change based on parameter in lin-reg.ipynb
static = False # This needs to change based on parameter in lin-reg.ipynb
    ...
    for g in ips_df.GeoID.unique():
        ...
        if static:
            ...
            if countryName != 'United States':
                ...
                continue
            elif '_NaN' in g or '_nan' in g:
                # print("no static data for: ", g)
                continue
            print(countryName,regionName, g)
            census_gdf = census_data[census_data['Qualifying Name'] == regionName]
            lodes_gdf = lodes_data[lodes_data['Qualifying Name'] == regionName]
            all_census_data = np.array(census_gdf[census_cols])
            all_lodes_data = np.array(lodes_gdf[lodes_cols])
        ...
        while current_date <= end_date:
            ...
            if static:
                ...
                X = np.concatenate([X_cases.flatten(), X_npis.flatten(), X_census.flatten(), X_lodes.flatten()])
            else:
