...
def predict_df(start_date_str: str, end_date_str: str, path_to_ips_file: str, verbose=False, NB_LOOKBACK_DAYS = 30, rolling=False, static=False):
    ...
    for g in ips_df.GeoID.unique():
        ...
        while current_date <= end_date:
            # Prepare data
            X_cases = past_cases[-NB_LOOKBACK_DAYS:, 1]
            ...
            pred_rolling = model.predict(X.reshape(1, -1))[0]
            # Convert rolling avg to actual cases
            pred_cases = 7*(pred_rolling-past_cases[-1, 1])+past_cases[-7, 0]
            pred_cases = max(0, pred_cases)  # Do not allow predicting negative cases
            # Add if it's a requested date
            if current_date >= start_date:
                geo_preds.append(pred_cases)
                if verbose:
                    print(f"{current_date.strftime('%Y-%m-%d')}: {pred_cases}")
            else:
                if verbose:
                    print(f"{current_date.strftime('%Y-%m-%d')}: {pred_cases} - Skipped (intermediate missing daily cases)")

            # Append the prediction and npi's for next day
            # in order to rollout predictions for further days.
            new_row = np.array([pred_cases, pred_rolling])
