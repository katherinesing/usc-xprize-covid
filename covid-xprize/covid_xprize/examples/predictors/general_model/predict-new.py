...
MODEL_FILE = os.path.join(ROOT_DIR, "models", "mlp_model_0110_6.pkl")
...
def predict(start_date: str,
    ...
    """
    # !!! YOUR CODE HERE !!!
    preds_df = predict_df(start_date, end_date, path_to_ips_file, verbose=False, NB_LOOKBACK_DAYS=nb_lookback_days, rolling=rolling, transform=transform, fs=fs)
...
def predict_df(start_date_str: str, end_date_str: str, path_to_ips_file: str, verbose=False, NB_LOOKBACK_DAYS = 30, rolling=False, transform=False, fs=False):
    """
    ...
    if transform:
        # Load pipe_cases
        with open(CASES_PIPE_FILE, 'rb') as cases_pipe_file:
            cases_pipe = pickle.load(cases_pipe_file)
        hist_cases_df['NewCases'] = apply_transform(cases_pipe, hist_cases_df['NewCases'].to_numpy())
        hist_cases_df['7DMA'] = apply_transform(cases_pipe, hist_cases_df['7DMA'].to_numpy())
    ...
    if os.path.exists(FS_INDEX_FILE):
        with open(FS_INDEX_FILE, 'rb') as f:
            fs_index = np.load(f,allow_pickle=True)
    ...
    for g in ips_df.GeoID.unique():
        ...
        while current_date <= end_date:
            # Prepare data
            X_cases = past_cases[-NB_LOOKBACK_DAYS:, 1] if rolling else past_cases[-NB_LOOKBACK_DAYS:, 0]
        ...
        geo_pred_df['PredictedDailyNewCases'] = inverse_transform(cases_pipe, np.array(geo_preds,dtype=np.float64)) if transform else geo_preds
